...
// The cursor is the one piece of global state this process changes: enter()
// detaches it from the mouse and hides it. Leaving it that way looks exactly
// like a frozen Mac, so every exit path puts it back.
atexit {
    CGAssociateMouseAndMouseCursorPosition(1)
    CGDisplayShowCursor(CGMainDisplayID())
}
